const { Reporte, Agente_Causal, Ruta_Transmision, Estado, Localidad, Tipo_Patogeno, Grupo_Riesgo, Usuario, Medida_Tiempo } = require('../models/associations');
...
const update = async (req, res) => {
  try {
    ...
    //IMPORTANTE
    if (req.body.uci) updateData.numero_casos_uci = req.body.uci;
...
const getAllReportsPendientes = async (req, res) => {
  try {
    ...
    const reportesFormateados = reportes.map(reporte => ({
      ...
      localidad: reporte.UbicacionReporte ? reporte.UbicacionReporte.localidad : null,
      estado: reporte.UbicacionReporte?.Estado?.estado || null,
      pais: reporte.UbicacionReporte?.Estado?.Pais?.pais || null,
      ...
      agente_causal: reporte.Agente_Causal ? reporte.Agente_Causal.agente_causal : null,
      ...
      id_medida_dpi: reporte.MedidadpiReporte?.medida || null,
      id_medida_dpe: reporte.MedidadpeReporte?.medida || null,
...
const getByIdUsuario = async (req, res) => {
  try {
    ...
    let agenteConditions = {};
    let estadoConditions = {};
    let usuarioConditions = {};
    let paisConditions = {}; 
    const { id_usuario, titulo, ruta_transmision, tipo, grupo_riesgo, agente_causal, estado, pais } = req.query;
    const searchTermLowerCase1 = (titulo || '').toLowerCase(); // Convertir el término de búsqueda a minúsculas
    if (id_usuario) {
      usuarioConditions = {
